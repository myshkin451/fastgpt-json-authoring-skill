...
- 支持真实导出中常见节点：
  ...
  `classifyQuestion`, `contentExtract`, `cfr`, `datasetConcatNode`,
  `readFiles`, `code`, `loop`, `parallelRun`, `loopStart`, `loopEnd`,
  `tools`, `tool`, `toolSet`, `toolParams`, `stopTool`, `pluginModule`,
  `customFeedback`。
- 支持生产级关注点：
  token 脱敏、知识库重绑定、HTTP header 插值、form-data、动态输出、菜单回流、
  运行态变量、分类分支、内容提取字段输出、代码节点返回值、批量/并行子流程、
  工具调用 selectedTools 连线。
